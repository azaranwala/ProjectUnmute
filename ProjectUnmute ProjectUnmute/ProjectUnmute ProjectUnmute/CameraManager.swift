...
class DeviceCameraManager: NSObject, ObservableObject {
    ...
    func switchCamera(to source: CameraSource) {
        ...
        let isSameSource = (cameraSource == source)
        let isAlreadyStreaming = (state == .streaming)
        
        // If same source and already streaming, no need to restart
        if isSameSource && isAlreadyStreaming && previewLayer != nil {
            print("📷 Already streaming from \(source.rawValue), skipping restart")
            return
        }
