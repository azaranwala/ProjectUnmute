...
final class ASLSignDetector: ObservableObject {
    ...
    @Published var useMLClassifier: Bool = true  // Use trained ML model for detection
    ...
    func processFrame(_ frame: CGImage) {
        ...
        do {
            ...
            guard let observation = handPoseRequest.results?.first else {
                // No hand detected - reset temporal state for clean start when hand returns
                DispatchQueue.main.async {
                    self.isDetecting = false
                    self.handVisible = false
                    self.detectedSign = nil
                    // Reset classifier's temporal smoothing when hand leaves frame
                    ASLModelClassifier.shared.resetTemporalState()
                    // Also reset motion detector
                    MotionSignDetector.shared.reset()
                }
                return
            }
    ...
    private func extractLandmarks(from observation: VNHumanHandPoseObservation) -> [(x: Float, y: Float, z: Float)] {
        ...
        for jointName in jointNames {
            if let point = try? observation.recognizedPoint(jointName) {
                ...
                // ALWAYS mirror X to match training data (collected with cv2.flip mirrored frames)
                // This applies to ALL cameras because the ML model expects mirrored coordinates
                // Invert Y for consistency with MediaPipe coordinate system
                landmarks.append((
                    x: Float(1.0 - point.location.x), // Mirror X to match training data
    ...
    func speakSentence() {
        ...
        // Get selected language and translate each word in the sentence
        let language = LanguageSettingsManager.shared.selectedLanguage
        let words = detectedSentence.components(separatedBy: " ")
        let translatedWords = words.map { language.translatedPhrase(for: $0) }
        let translatedSentence = translatedWords.joined(separator: " ")
        
        // Create utterance with translated text
        let utterance = AVSpeechUtterance(string: translatedSentence)
        utterance.rate = AVSpeechUtteranceDefaultSpeechRate * 0.9 // Slightly slower
        utterance.pitchMultiplier = 1.0
        utterance.volume = 1.0
        
        // Use voice for selected language with fallback
        if let voice = findBestVoice(for: language) {
            utterance.voice = voice
        }
        
        speechSynthesizer.speak(utterance)
        logger.info("Speaking: \(self.detectedSentence) as '\(translatedSentence)' in \(language.displayName)")
    }
    
    /// Reset detector state (for mode switching)
    func reset() {
        detectedSign = nil
        detectedSentence = ""
        confidence = 0
    ...
    private func speakSign(_ sign: String) {
        ...
        // Get the selected language and translate the sign
        let language = LanguageSettingsManager.shared.selectedLanguage
        let translatedPhrase = language.translatedPhrase(for: sign)
        
        let utterance = AVSpeechUtterance(string: translatedPhrase)
        utterance.rate = AVSpeechUtteranceDefaultSpeechRate
        utterance.pitchMultiplier = 1.0
        utterance.volume = 1.0
        
        // Try to get a voice for the selected language, with fallbacks
        if let voice = findBestVoice(for: language) {
            utterance.voice = voice
            logger.info("Using voice: \(voice.language) - \(voice.name)")
        } else {
            logger.warning("No voice found for \(language.displayName), using default")
        }
        ...
        speechSynthesizer.speak(utterance)
        logger.info("🔊 Auto-speaking sign: \(sign) as '\(translatedPhrase)' in \(language.displayName)")
    }
    
    /// Find the best available voice for the selected language
    private func findBestVoice(for language: LanguagePreference) -> AVSpeechSynthesisVoice? {
        let languageCode = language.ttsLanguageCode
        ...
        // Get all available voices and find one that matches the language prefix
        let allVoices = AVSpeechSynthesisVoice.speechVoices()
        let languagePrefix = String(languageCode.prefix(2)) // "en", "es", "zh"
        
        // Try to find any voice matching the language
        for voice in allVoices {
            if voice.language.hasPrefix(languagePrefix) {
                logger.info("Found fallback voice: \(voice.language)")
                return voice
            }
        }
        
        // Log available voices for debugging
        let availableLanguages = Set(allVoices.map { $0.language }).sorted()
        logger.warning("Available TTS languages: \(availableLanguages.joined(separator: ", "))")
    ...
    private func detectSign(from landmarks: [(x: Float, y: Float, z: Float)]) -> DetectedSignResult? {
        ...
        let pinkyExtended = isFingerExtended(landmarks, tip: LM.pinkyTip, pip: LM.pinkyPIP, isPinky: true)
    ...
    private func isFingerExtended(_ landmarks: [(x: Float, y: Float, z: Float)], tip: Int, pip: Int, isPinky: Bool = false) -> Bool {
