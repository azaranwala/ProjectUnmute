...
struct ContentView: View {
    ...
    @StateObject private var languageSettings = LanguageSettingsManager.shared
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                // Speech→ASL mode: No camera needed, just speech recognition and avatar
                // ASL→Speech mode: Needs camera for sign detection
            ...
            .onChange(of: communicationMode) { _, newMode in
                ...
                if newMode == .speechToASL {
                    // Stop camera streaming when switching to Speech→ASL (no camera needed)
                    cameraManager.stopStreaming()
                } else {
                    // Reset avatar view when switching to ASL→Speech
    ...
    private var languageButton: some View {
        Button(action: { showLanguagePicker = true }) {
            Text(languageSettings.selectedLanguage.flag)
                .font(.title3)
        }
        .sheet(isPresented: $showLanguagePicker) {
            LanguagePickerSheet(selectedLanguage: $languageSettings.selectedLanguage)
    ...
    // MARK: - Speech → ASL View (No camera needed - speech to avatar only)
    ...
    private var aslToTextView: some View {
            ...
            VStack(spacing: 12) {
                ...
                GeometryReader { geometry in
                    ZStack {
                        ...
                        if cameraManager.cameraSource == .metaGlasses {
                            // Meta Glasses - only show video when frames are available
                            if let frame = cameraManager.currentFrame {
                                MWDATVideoView(frame: frame)
                            } else {
                                // Blank placeholder until glasses stream is ready
                                ZStack {
                                    Color.black
                                    VStack(spacing: 16) {
                                        Spacer()
                                        ...
                                        Text("Waiting for Meta Glasses...")
                                            .font(.caption)
                                            .foregroundColor(.gray)
                                        
                                        Spacer()
                                        
                                        // Authorize button at bottom - only if not authorized
                                        if !MetaGlassesCameraManager.shared.isAuthorized {
                                            Button(action: {
                                                Task {
                ...
                ASLDemoPanel(aslDetector: aslDetector)
                    .padding(.horizontal)
        ...
        .onAppear {
            // Auto-start front camera when ASL-to-Text view appears
