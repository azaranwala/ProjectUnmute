...
final class AvatarVideoManager: ObservableObject {
    ...
    func playVideo(for phrase: String) {
        ...
        // Always try to set the video name - let the player handle finding it
        // Check if video exists in our list first
        if availableVideos.contains(normalized) {
            ...
            // If no videos were scanned, try playing anyway
            currentVideoName = normalized
            ...
            if !found {
                // Still try to play - maybe the scan missed it
                currentVideoName = normalized
                logger.info("No exact match, trying to play anyway: \(normalized)")
            }
        }
    }
